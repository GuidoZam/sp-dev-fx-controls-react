...
export class PaletteGenerator {
  ...
  public static GetPalette(
    ...
    switch (palette) {
      case ChartPalette.OfficeColorful4:
        paletteColors = this.generateRepeatingPattern(
          OFFICE_COLORFUL4,
          desiredLength
        );
        break;
      ...
      case ChartPalette.OfficeMonochromatic1:
        paletteColors = this.generateNonRepeatingGradient(
          OFFICE_MONOCHROMATIC1,
          desiredLength
        );
        break;
   ...
   */
  public static generateRepeatingPattern(
    array: string[],
    desiredLength: number
  ): string[] {
    if (desiredLength === 0) {
   ...
   */
  public static generateNonRepeatingGradient(
    colorRange: string[],
    numSteps: number
  ): string[] {
    ...
    for (let index: number = 0; index < numSteps; index++) {
      const interpolatedColor: number[] = this._interpolateColors(
        startRGB,
        endRGB,
        factorStep * index
      );
